...
function updateAnalytics() {
  ...
  const avgProfit = Math.round(totalProfit / flights.length);
  ...
  grid.innerHTML = `
    ...
    <div class="analytics-card">
      <div class="analytics-label">${L.analyticsAvgProfit || 'Avg Profit/Flight'}</div>
      <div class="analytics-value">${fmtMoney(avgProfit)}</div>
      <div class="analytics-sub">${flights.length} ${L.statFlights || 'flights'}</div>
    </div>
    <div class="analytics-card">
...
function updateMap() {
  ...
  flights.forEach(f => {
    ...
    const line = L.polyline([from, to], { color, weight: 3, opacity: 0.7 });
    line.bindPopup(`
    ...
    [from, to].forEach(pos => {
      const mk = L.circleMarker(pos, { radius: 5, fillColor: '#3b82f6', color: '#1d4ed8', fillOpacity: 1, weight: 2 });
...
async function saveSettings() {
  try {
    ...
    await Promise.all([
      API.saveSetting('pricing', pricing),
      API.saveSetting('language', lang),
      simbriefId ? API.saveSetting('simbriefId', simbriefId) : Promise.resolve(),
      reportEmail ? API.saveSetting('reportEmail', reportEmail) : Promise.resolve()
    ]);
...
async function resetSettings() {
  ...
  await API.saveSetting('pricing', pricing);
