...
// Default missions - hardcoded in Hebrew
...
router.get('/', async (req, res) => {
  try {
    ...
    // החזר DEFAULT_MISSIONS כמקור האמת (עם הקידוד הנכון)
    // סנן משימות עתידיות או משימות ללא תאריך או משימות שהושלמו
    const missions = DEFAULT_MISSIONS.filter(m => {
      if (!m.date || m.date >= today) return true;  // משימות עתידיות או ללא תאריך
      if (completedIds.includes(m.id)) return true;  // משימות שהושלמו
      return false;
    });
